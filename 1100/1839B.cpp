...
cin>>t;
while(t--){ans=0;
cin>>n;int a[n],b[n];fill(a,b,n);
map<int,int> m;
heap(a,b,n);
vector<vector<int>>v;vector<int> aux;
for(int i=0;i<n;i++)v.push_back(aux);
for(int i=0;i<n;i++){v[a[i]-1].push_back(b[i]);
	
}
set<int> p;for(int i=0;i<n;i++){p.insert(a[i]);
	
}vector<int>pp;
for(int k:p)pp.push_back(k);
for(int i=0;i<pp.size();i++){
sort(v[pp[i]-1].begin(),v[pp[i]-1].end());
}
for(int i=0;i<pp.size();i++){ll c=0;
for(int j=0;j<pp[i];j++){
	if(j==v[pp[i]-1].size())break;
	int l=v[pp[i]-1].size();
	c+=v[pp[i]-1][l-j-1];
}ans+=c;
}cout<<ans<<endl;

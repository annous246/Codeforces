...
int minv(vector<int> a,int n){int m=1000000001;
for(int i=0;i<n;i++){
	if(a[i]<m){m=a[i];
	}
}return m;
}
int maxv(vector<int> a,int n){int m=0;
for(int i=0;i<n;i++){
	if(a[i]>m){m=a[i];
	}
}return m;
}
void back(int ind,vector<vector<int>>&s,vector<int> &temp,int a[],int n,int x,int l,int r,ll sum,vector<int> &sv){
if(temp.size()&&(maxv(temp,temp.size())-minv(temp,temp.size())>=x&&sum>=l&&sum<=r)){
	vector<int> aux=temp;
	sort(aux.begin(),aux.end());
	s.push_back(temp);
}
if((maxv(temp,temp.size())-minv(temp,temp.size())<x||sum>r)&&temp.size()>n){
	return;
}
	for(int i=ind;i<n;i++){
		temp.push_back(a[i]);
		sv[i]=1;
		back(i+1,s,temp,a,n,x,l,r,sum+(ll)a[i],sv);
		sv[i]=0;
		temp.pop_back();
	}
}
int main(){
int t,n,l,r,x;ll ans;ans=0;
cin>>n>>l>>r>>x;int a[n];fill(a,n);
heapSort(a,n);
vector<vector<int>>s;
vector<int> temp;
vector<int> sv;
for(int i=0;i<n;i++)sv.push_back(0);
back(0,s,temp,a,n,x,l,r,0,sv);
cout<<s.size();

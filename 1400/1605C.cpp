...
int t,n,k;ll ans;
cin>>t;
while(t--){ans=0;string s;
cin>>n>>s;vector<int>v;
for(int i=0;i<n-1;i++){
	if(s[i+1]=='a'&&(s[i+1]==s[i])){
		cout<<2;goto e;
	}
}
for(int i=0;i<n;i++){
...
if(v.size()<=1)cout<<-1;
	...
	if(v[i+1]-v[i]==3&&v[i+1]-v[i]==v[i+2]-v[i+1]){
			if(s[v[i+1]-1]==s[v[i]+1]&&s[v[i+2]-1]==s[v[i+1]+1]&&s[v[i]+1]!=s[v[i+1]+1]){
				cout<<7;goto e;
			}
		}

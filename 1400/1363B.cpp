...
int t,n,k;ll ans;
cin>>t;
while(t--){ans=10000;string s;
cin>>s;int n=s.length(),z=0,o=0;
for(char c:s){if(c=='0')z++;
else o++;
}
int oo=o,zz=z,c=0,ok=0;o=0;
//first part zeroes
for(int i=0;i<n;i++){
	char  k=s[i];
	if(k=='1')ok=1;
	if(ok){
		if(k=='1'){
			c=z+o;ans=min(ans,c);o++;
		}
		else{z--;c=o+z;ans=min(ans,c);
			
		}
	}else z--;
	
}z=0;o=oo;c=0;ok=0;//cout<<ans<<endl;
//first part ones
for(int i=0;i<n;i++){
	char  k=s[i];
	if(k=='0')ok=1;
	if(ok){
		if(k=='0'){
			c=o+z;ans=min(ans,c);z++;
		}
		else{o--;c=o+z;ans=min(ans,c);

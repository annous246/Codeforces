...
int main(){
int t,n,k;ll ans;ans=0;string ss;cin>>ss;n=ss.length();
pair <int,int> f(-1,-1),s(-1,-1);int ok=2,op=1;
for(int i=0;i<n-1;i++){
	if(ss[i]=='A'){if(ss[i+1]=='B'){
		if(ok>1){
		f.first=i;ok=0;
		 }s.second=i;
	}}
	else if(ss[i]=='B'){if(ss[i+1]=='A'){
	   if(op)	{s.first=i;op=0;
	   }
...
if(f.first!=-1&&f.second!=-1){
	if(f.first+1<f.second){cout<<"YES"<<endl;goto v;
	}
}
else goto e;
if(s.first!=-1&&s.second!=-1){
	if(s.first+1<s.second){cout<<"YES"<<endl;goto v;
	}
}
e:cout<<"NO";cout<<endl;v:;

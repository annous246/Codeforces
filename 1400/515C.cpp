...
int main(){
int t,n,k;ll ans;ans=0;string s;
cin>>n>>s;vector<int> v,m;
for(char c:s){int i=(int)c-48;
v.push_back(i);
}
sort(v.begin(),v.end());
int *p=new int[10]();
for(int i=0;i<n;i++){
	switch(v[i]){
		case 2:p[2]++;break; 
		case 3:p[3]++;break; 
		case 4:p[2]+=2;p[3]++;break; 
		case 5:p[5]++;break; 
		case 6:p[5]++;p[3]++;break; 
		case 7:p[7]++;break; 
		case 8:p[2]+=3;p[7]++;break; 
		case 9:p[7]++;p[3]+=2;p[2]++;break; 
	}
}
for(int l=0;l<10;l++){
	while(p[l]--)
	m.push_back(l);
}sort(m.begin(),m.end());for(int i=m.size()-1;i>-1;i--)cout<<m[i];
